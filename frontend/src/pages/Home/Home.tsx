...
import { EventTypes, type EventHandler, type OnFormSubmittedEvent, type OnLocationClickedEvent } from '../../SystemEventHub'
    ...
    () => {
      ...
      function handleFormSubmitted (evt: OnFormSubmittedEvent): void {
        ...
        SystemServices.getWeatherFromCity(evt.city)
          .then(wr => { setWeatherResponse(wr) })
          .catch(err => { console.error(err) })

      }

      function handleLocationClick (evt: OnLocationClickedEvent): void {

        SystemServices.getWeatherFromCoordinates(evt.location.coordinates)
          .then(wr => { setWeatherResponse(wr) })
          .catch(err => { console.error(err) })
      ...
      SystemEventHub.on(EventTypes.locationClicked, handleLocationClick as EventHandler)

      return () => {

        SystemEventHub.off(EventTypes.formSubmitted, handleFormSubmitted as EventHandler)
        SystemEventHub.off(EventTypes.locationClicked, handleLocationClick as EventHandler)
